...
class DuelManager extends BaseEntityManager
{
    ...
    public function findAllRelatedToUser(User $user)
    {
        $qb = $this->getRepository()->createQueryBuilder('duel')
            ...
            ->leftJoin('initiator.avatar', 'initiator_avatar')
            ->leftJoin('initiator.department', 'initiator_department')
            ->leftJoin('initiator.userAchievements', 'initiator_user_achievements')
            ->leftJoin('duel.victim', 'victim')
            ->leftJoin('victim.avatar', 'victim_avatar')
            ->leftJoin('victim.userAchievements', 'victim_user_achievements')
            ->leftJoin('victim.department', 'victim_department')
